---
title: MCP Server
description: 'Developer guide: control Kakiyo via AI assistants like Claude, Cursor, and other MCP clients. This page is for programmatic access.'
---

<Info>
  **This page is for developers.** It covers the MCP server for AI assistant integrations. If you're looking for how to use Kakiyo from the dashboard (no code), see the [Dashboard Guides](/guides/overview) instead.
</Info>
...
<CardGroup cols={2}>
  <Card title="OAuth (Claude custom connectors)" icon="shield-check">
  ...
  <Card title="API key header (Bearer)" icon="key">
    Best for Claude Code CLI, Claude Desktop via config file, Cursor, OpenCode, and any MCP client that supports HTTP headers. Uses a 40-character team-scoped API key.
  </Card>
</CardGroup>
...
<Tabs>
  ...
  <Tab title="Claude.ai (Web & Desktop)">
    ...
    <Frame caption="Kakiyo OAuth consent screen for Claude MCP connector">
      <img
        src="https://public-kakiyo.b-cdn.net/Integrations/Screenshot%202026-04-23%20at%2010.27.31%E2%80%AFPM.png"
        alt="Kakiyo x Claude MCP connector consent screen"
      />
    </Frame>
    ...
    <Warning>
      Claude custom connectors do not support pasting a raw API key in the connector dialog. Use the OAuth flow above for `claude.ai`/Claude Desktop, or use the **Claude Code**, **Claude Desktop config file**, **Cursor**, or **OpenCode** tabs below for header-based API-key setups.
    </Warning>
  </Tab>
  <Tab title="OpenCode">
...
<Tabs>
  <Tab title="OAuth 2.1 (recommended for Claude)">
    Claude custom connectors use the standard MCP OAuth flow. When Claude calls `/mcp` without a valid token, Kakiyo responds with:

    ```
    HTTP/1.1 401 Unauthorized
    WWW-Authenticate: Bearer error="invalid_token",
                             resource_metadata="https://api.kakiyo.com/.well-known/oauth-protected-resource/mcp",
                             scope="mcp:read mcp:write"
    ```

    Kakiyo exposes the standard MCP/OAuth metadata endpoints:

    | Endpoint | Purpose |
    |----------|---------|
    | `GET /.well-known/oauth-protected-resource/mcp` | Protected resource metadata ([RFC 9728](https://datatracker.ietf.org/doc/html/rfc9728)) |
    ...
    - Resource Indicators ([RFC 8707](https://www.rfc-editor.org/rfc/rfc8707.html)) targeting `https://api.kakiyo.com/mcp`
    - Refresh tokens with rotation
    - Team-scoped access tokens (approval binds the connector to one Kakiyo team)
  </Tab>
  <Tab title="API key (Bearer header)">
    Legacy and programmatic clients can authenticate with a 40-character team-scoped API key:

    ```
    Authorization: Bearer {your_40_char_api_key}
    ```

    API keys continue to work for Claude Code CLI, Claude Desktop config files, Cursor, OpenCode, and any other MCP client that allows custom HTTP headers.
  </Tab>
</Tabs>
...
<AccordionGroup>
  <Accordion title="Claude says 'Couldn't reach the MCP server'">
  ...
  <Accordion title="Claude OAuth flow opens but approval fails">
  ...
  <Accordion title="Connection refused or timeout (non-Claude clients)">
